...
\begin{itemize}
    \item KNN (en \texttt{small} et \texttt{medium})
    \item Random Forest (en \texttt{small} et \texttt{medium})
    \item XGBoost (en \texttt{small} et \texttt{medium})
    \item MLP (en \texttt{small})
\end{itemize}
...
\begin{itemize}
	\item Tab Transformer (en \texttt{small}) : TO COMPLETE
	\item FT Transformer (en \texttt{small}) : TO COMPLETE
	\item MLP avec attention (en \texttt{small}) : même architecture que le MLP classique, mais avec un mécanisme d'attention ajouté.
\end{itemize}
...
En effet, pour chaque type de modèle implémenté, deux tailles de modèles ont été testées : \texttt{small} et \texttt{medium}.
La différence principale entre elles réside dans le nombre de couches et de neurones utilisés.
Par exemple, pour le MLP, le modèle \texttt{small} utilise 2 couches cachées avec 32 et 64 neurones, tandis que le modèle \texttt{medium} utilise 3 couches cachées de 64, 128 et 256 neurones.

% Brouillon
BROUILLON : Pour les hyperparamètres, pour l'instant, ont juste été testés 2 tailles de modèles (small et medium) pour chaque dataset, et utilisés des bons learning rate et autres hyperparamètres par défaut, et de bons résultats ont été obtenus.
...
En s'attardant sur les erreurs commises par les différents modèles, on peut observer certaines corrélation entre celles-ci, comme le montre la \hyperref[fig:corrmat_physical]{Figure 1}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{images/correlation_heatmap_physical.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{images/correlation_heatmap_network.png}
