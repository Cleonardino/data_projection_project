...
\newcommand{\verticalquote}[1]{%
    \begin{tikzpicture}[remember picture, overlay]
        \draw[thick,black] (-0.5cm,0) -- (-0.5cm,-0.4cm);  % Ligne verticale
    \end{tikzpicture}%
...
\usepackage{csquotes}
\usepackage[
    backend=biber,   % utilise biber au lieu de bibtex
    style=numeric,   % citations numérotées
    sorting=none     % ordre du .bib
]{biblatex}
...
\newpage
\pagenumbering{roman} % réactiver la numérotation
...
Chaque ligne est associée à une étiquette (\texttt{label}) indiquant si les données correspondent à un état normal (étiquette \texttt{normal}) ou à une anomalie (étiquettes \texttt{DoS}, \texttt{MITM}, \texttt{scan} pour les données réseau et \texttt{physical fault} pour les données physiques).

\myparagraph{Données physiques}

En s'appuyant sur les informations fournies par \cite{article_reference}, on peut dénombrer les nombre de lignes pour chaque jeu de données physique comme présenté dans la \hyperref[fig:phys_number_entries]{Figure 1}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{images/phys_number_entries.png}
...
En tout, il y a 41 caractéristiques, présentées dans la \hyperref[tab:physical_features]{Table 2} (une traduction du tableau original présent dans \cite{article_reference}).
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{images/net_number_entries.png}
	\caption{Nombre d'entrées par fichier de données réseau d'après \cite{article_reference}}
	\label{fig:net_number_entries}
\end{figure}

Au nombre de 14, les colonnes des données réseau sont listées dans la \hyperref[tab:network_features]{Table 3} (une traduction du tableau original présent dans \cite{article_reference}).
...
\begin{enumerate}
	...
	\item Les labels sont encodés avec \texttt{LabelEncoder} \cite{scikit-learn_labelencoder} de \texttt{scikit-learn} \cite{scikit-learn_article}, ce qui permet de convertir les étiquettes textuelles en valeurs numériques.
	\item Les données sont stratifiées en ensembles d'entraînement, de validation et de test avec \texttt{train\_test\_split} \cite{scikit-learn_train_test_split} de \texttt{scikit-learn}, en utilisant les options de \texttt{stratify} et \texttt{shuffle} pour garantir une répartition équilibrée des classes dans chaque ensemble.
		  Le ratio utilisé est de 70\% pour l'ensemble d'entraînement, 15\% pour l'ensemble de validation et 15\% pour l'ensemble de test.
	\item Si le \textit{balancing} est activé, il est appliqué sur l'ensemble d'entraînement à l'aide de la fonction \texttt{RandomOverSampler} \cite{imbalanced-learn_randomoversampler} de la bibliothèque \texttt{imbalanced-learn} \cite{imbalanced-learn}.
	% \item Si le balancing est activé, il est appliqué sur l'ensemble d'entraînement à l'aide de différentes techniques (notamment via des fonctions de la bibliothèque \texttt{imbalanced-learn} \cite{imbalanced-learn}) :
	% \begin{itemize}
		% \item \textbf{OverSampling} avec \texttt{SMOTE} \cite{imbalanced-learn_smote}
		% \item \textbf{OverSampling} avec \texttt{RandomOverSampler} \cite{imbalanced-learn_randomoversampler}
		% \item \textbf{OverSampling} manuellement avec l'augmentation des données
		% \item \textbf{UnderSampling} avec \texttt{RandomUnderSampler} \cite{imbalanced-learn_randomundersampler}
	% \end{itemize}
...
\begin{itemize}
	\item Tab Transformer : TO COMPLETE
	\item FT Transformer : TO COMPLETE
	\item MLP avec attention : même architecture que le MLP classique, mais avec un mécanisme d'attention ajouté.
\end{itemize}

\subsection{Organisation des entraînements}

% À réviser, actuellement généré par IA

L'ensemble du code est organisé pour permettre une expérimentation facile et reproductible des différents modèles sur les deux types de données (physiques et réseau).
Chaque modèle est défini dans un script séparé contenu dans un dossier \texttt{models/}, avec des configurations spécifiques pour chaque type de modèle.
Les données sont pré-traitées de manière cohérente pour tous les modèles, et un système de cache est mis en place pour éviter de répéter le pré-traitement lors de nouvelles exécutions.
Les résultats de chaque entraînement sont enregistrés de manière structurée, permettant une analyse et une comparaison faciles des performances des modèles.
...
\begin{itemize}
	...
	\item Un système de cache est utilisé pour éviter de traiter à nouveau les données lors de nouvelles exécutions et sont enregistrées dans le dossier \texttt{cached\_datasets}.
	\item Ensuite, les modèles sont entrainés individuellement avec le script \texttt{scripts/train.py} sur un fichier de config, ou alors il est aussi possible d'entraîner tous les modèles ou un groupe de modèles d'un coup avec le script \texttt{scripts/train\_all.py}.
	\item Les résultats de chaque entraînement de chaque config de modèle sont enregistrés dans le dossier \texttt{experiments}.
	\item Sont enregistrés pour chaque entraînement:
	\begin{itemize}
		\item les poids du meilleurs modèle
		\item la configuration utilisée (au cas ou les configurations changent)
		\item Les métriques sur le jeu de données d'entraînement, validation et test du meilleur modèle.
		\item Les erreurs sur le jeu de données d'entraînement, validation et test du meilleur modèle.
		\item Les courbes de loss et d'accuracy sur le jeu de donnée d'entraînement et de validation.
	\end{itemize}
	\item Un rapport récapitulatif de tous les entraînements est ensuite généré avec le script \texttt{src/analyze\_results.py} séparé pour le physical et le network datasets.
\end{itemize}
%
...
En effet dans la \hyperref[tab:results_physical]{Table 5}, on peut voir que les performances des KNN, Random Forest, SVM et Naive Bayes (NB) sont TO COMPLETE.
...
La \hyperref[tab:misclassified_samples]{Table 6} liste les échantillons qui ont été mal classés par l'ensemble des modèles lors de nos différentes exécutions, ainsi que la prédiction majoritaire effectuée par les modèles pour ces échantillons.
