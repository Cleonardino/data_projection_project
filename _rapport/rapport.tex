...
La répartition globale peut être observée dans la \hyperref[fig:data_repartition]{Figure 1}.\\
...
\begin{itemize}
    \item KNN (en \texttt{small}, \texttt{medium} et \texttt{large})
    \item Random Forest (en \texttt{small},  \texttt{medium} et \texttt{large})
    \item XGBoost (en \texttt{small} et \texttt{medium})
    \item MLP (en \texttt{small}, \texttt{medium} et \texttt{large})
\end{itemize}

Le modèle CART n'a pas été implémenté, mais a été remplacé par les trois modèles ci-dessous :

\begin{itemize}
	\item Tab Transformer (en \texttt{small} et \texttt{medium}) : modèle basé sur des transformeurs appliqués aux données tabulaires, utilisant des embeddings pour les variables catégorielles et un mécanisme d’auto-attention pour capturer les interactions entre les caractéristiques.
	\item FT Transformer (en \texttt{small} et \texttt{medium}) : variante optimisée du transformeur pour données tabulaires, combinant embeddings de caractéristiques et couches d’auto-attention avec une tête de prédiction dédiée, offrant de bonnes performances tout en restant peu coûteuse en calcul.
	\item MLP avec attention (en \texttt{small}, en \texttt{medium} et \texttt{large}) : même architecture que le MLP classique, mais avec un mécanisme d'attention ajouté.
\end{itemize}
