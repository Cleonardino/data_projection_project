...
Il est également possible de remplacer l'un de ces modèles par un ou plusieurs autres au choix, ce qui a été fait en implémentant des modèles basés sur des transformeurs — Tab Transformer et Feature Tokenizer Transformer (FT Transformer) — ainsi qu'un MLP avec attention à la place du modèle CART.
...
\begin{figure}[H]
	...
	\caption{nombre de valeurs manquantes par colonne dans les données réseau}
...
\begin{enumerate}
	...
	\item Les valeurs manquantes (cf. \hyperref[sec:missing_values]{1.1.3 Valeurs manquantes}) sont remplacées par des 0.
...
Les données sont pré-traitées de manière cohérente pour tous les modèles (cf. \hyperref[label:preprocessing]{1.2 Pré-traitement}), et un système de cache est mis en place pour éviter de répéter le pré-traitement lors de nouvelles exécutions.
...
\begin{itemize}
	...
	\item un tableau récapitulatif du top 20 des entrées les moins bien classées sur l'ensemble des modèles, avec leur prédiction majoritaire,
...
L'évaluation sur le jeu de données réseau (\hyperref[tab:network_results]{Table 5}) montre une dispersion des performances beaucoup plus marquée que sur les données physiques. Avec plusieurs centaines de millier d'entrées, la capacité de généralisation des modèles est mise à l'épreuve. On observe que les modèles "small", bien que plus simples, dominent ici le classement. Cela suggère que l'augmentation de la complexité des modèles (tailles "medium" et "large") sans un réglage extrêmement fin des hyperparamètres ou une augmentation proportionnelle du temps d'entraînement mène rapidement à une dégradation des performances sur ce type de trafic.
...
\begin{table}[H]
	...
	\caption{Comparaison des expériences sur le jeu de données réseau}
    \label{tab:network_results}
\end{table}

En comparant nos résultats avec ceux de la \hyperref[tab:article_res_net]{Table 6}, fournis par \cite{article_reference}, on note une tendance intéressante : nos meilleurs modèles (\texttt{small\_random\_forest} et \texttt{small\_knn}) obtiennent une exactitude (accuracy) supérieure à celle rapportée dans l'article (0,87-0,89 contre 0,75-0,77). De même, nos F1-scores macro sont nettement plus élevés (0,86 contre 0,54).

Cette supériorité peut s'expliquer par notre stratégie de pré-traitement incluant le sous-échantillonnage et la normalisation, ou par une meilleure adaptation des hyperparamètres de nos modèles "small". Toutefois, il est important de noter que le temps de calcul des modèles complexes comme le \texttt{large\_mlp} ou le \texttt{ft\_transformer} explose sur ce dataset, sans pour autant garantir de meilleurs résultats, ce qui valide l'efficacité des méthodes arborescentes et des voisins les plus proches pour la détection d'intrusions réseau.\\
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		\hline
	\end{tabular}
	\caption{Résultats de l’évaluation des modèles sur le jeu de données réseau obtenus par \cite{article_reference}}
...
D'après les résultats présentés dans la \hyperref[tab:physical_results]{Table 7}, le modèle \texttt{large\_xgboost} obtient les meilleures performances sur le jeu de données \texttt{physical}, avec une précision de \textbf{98,60\%}, un F1-macro de \textbf{0,8179}, une précision équilibrée de \textbf{0,8174} et un MCC de \textbf{0,9609}.
...
En comparant les résultats obtenus avec ceux de \cite{article_reference}, on constate que nos modèles affichent des performances globales très compétitives en termes d'exactitude (accuracy), bien que le F1-score macro suggère une difficulté persistante à classifier parfaitement certaines classes minoritaires par rapport aux résultats de l'article.
En effet dans la \hyperref[tab:article_res_phy]{Table 8}, on peut voir que les performances des KNN, Random Forest, SVM et Naive Bayes (NB) sont sensiblement supérieures aux nôtres en termes de F1-score.
Cette différence peut provenir d'un prétraitement des données distinct, d'une gestion différente du déséquilibre des classes, ou de l'utilisation d'une moyenne pondérée (weighted) plutôt que macro pour le calcul du F1-score dans \cite{article_reference}.\\
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		\hline
	\end{tabular}
	\caption{Résultats de l’évaluation des modèles sur le jeu de données physiques obtenus par \cite{article_reference}}
...
En s'attardant sur les erreurs commises par les différents modèles, on peut observer certaines corrélation entre celles-ci, comme le montre les \hyperref[fig:corrmat_network]{Figure 7} et \hyperref[fig:corrmat_physical]{Figure 8}.
Par exemple — et sans surprise — les modèles avec des architectures similaires présentent une forte corrélation dans leurs erreurs.
...
Dans ce projet, j'ai principalement travaillé à développer l'architecture modulaire dont je suis très fier pour l'entraînement et le test des modèles, et le script qui va automatiquement analyser les résultats et les afficher dans un rapport markdown et les mettre à disposition de l'app streamlit.
...
Par contre, j'ai relevé un gros problème dans le papier associé au dataset, il n'y pas de code fourni accompagnant le papier avec toutes les indications pour pouvoir exactement reproduire les données du papier. On a donc par exemple un doute sur s'ils ont fait un F1-score macro ou un F1-score weighted. Personnelement, je ne comprends pas du tout comment un papier sans code publique peut être accepté dans une grande conférence (comme IEEE) sans fournir un code qui permet aux autres chercheurs de comparer et de comprendre pourquoi ils n'ont pas les mêmes résultats que ceux du papier, et on ne peut donc aussi absolument pas vérifier qu'ils n'aient pas triché d'une manière ou d'une autre.
