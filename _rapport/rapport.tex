...
Dans chaque fichier CSV, une ligne représente un ensemble de données collectées à un instant donné.
Les colonnes renvoient à différents mécanismes sur lesquels une mesure a été faite.
Elles sont présentées dans la \hyperref[tab:features]{Table 2}.
...
\begin{table}[H]
	...
	\begin{tabular}{c l l}
		...
		\hline
		feature\_1 & float & Description de la feature 1 \\
		feature\_2 & int   & Description de la feature 2 \\
		...        & ...   & ... \\
		label      & string & Type d'anomalie (ou normal) \\
		\hline
	\end{tabular}
	\caption{Description des features présentes dans les fichiers CSV}
	\label{tab:features}
\end{table}
...
\begin{enumerate}
	...
	\item Stratification de l'ensemble des données en ensembles d'entraînement, de validation et de test avec \textit{train_test_split} de \textit{scikit-learn} avec les options de \textit{stratify} et \textit{shuffle} avec un ratio de 70\% - 15\% - 15\% pour l'ensemble d'entraînement, de validation et de test.
...
\begin{itemize}
	...
	\item Différentes configurations pour chaque modèle (hyperparamètres, datasets, taille des modèles, etc.) dans \texttt{configs/model_type/config_name.yaml}.
